...
func (s *DistStore) EnqueueDue(ctx context.Context, job publish.Job) error {
	return s.inTenant(ctx, job.TenantID, func(db DB) error {
		_, err := db.Exec(ctx, `
UPDATE distribution_jobs
SET status = 'QUEUED', lease_until = NULL, lease_owner = '', updated_at = now()
WHERE id = $1 AND tenant_id = $2 AND status = 'SCHEDULED'`, job.ID, job.TenantID)
		return mapDB(err)
	})
}
...
UPDATE distribution_jobs SET
    ...
    next_attempt_at = CASE WHEN $3 = 'RETRY_WAITING' THEN $7 ELSE NULL END,
    attempt_count = CASE WHEN $4 IN ('KILL_SWITCH_ENGAGED','PUBLISH_POLICY_UNAVAILABLE') THEN GREATEST(attempt_count - 1, 0) ELSE attempt_count END,
    lease_until = NULL,
    lease_owner = '',
    updated_at = now()
WHERE id = $1 AND tenant_id = $2`,
			job.ID, job.TenantID, string(job.Status), attempt.ErrorCode, attempt.ErrorText, attempt.ExternalID, nextAttemptAt)
		if err != nil {
			return mapDB(err)
		}
		id, err := newID()
