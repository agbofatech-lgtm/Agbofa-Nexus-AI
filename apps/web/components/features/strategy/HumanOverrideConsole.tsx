...
export function HumanOverrideConsole({
  ...
  additionalTargets?: readonly { id: string; label: string }[];
  ...
  const targets = useMemo(
    ...
    [additionalTargets, plans, workforce],
  );
  ...
  const applyExtended = (label: string, requestedState: string) =>
    setMessage(`${label}: ${requestedState} for ${target} in local UI state. No policy, autonomy level, agent, budget, or backend enforcement changed.`);
  return (
    <section className="override-console" aria-labelledby="override-console-title">
      ...
      <div className="override-console__controls">
        ...
        <div><button onClick={() => apply("PAUSE")} type="button"><Pause aria-hidden="true" size={13} /> Simulate pause</button><button onClick={() => apply("STOP")} type="button"><Square aria-hidden="true" size={13} /> Simulate stop</button><button onClick={() => apply("OVERRIDE")} type="button"><Hand aria-hidden="true" size={13} /> Simulate override</button>{additionalActions.map((action) => <button key={action.id} onClick={() => applyExtended(action.label, action.requestedState)} type="button"><Hand aria-hidden="true" size={13} />{action.label}</button>)}</div>
      </div>
