  ...
  return (
    <div className="executive-operating-state">
      <section className="executive-metrics" aria-labelledby="executive-metrics-title">
        ...
        <div>{metrics.map((metric) => <article key={metric.id}><header><span>{metric.classification}</span><ExecutionRealityBadge reality={metric.executionReality} /></header><strong>{metric.displayValue}</strong><p>{metric.label}</p><small>{metric.context}</small><footer><DataSourceIndicator provenance={metric.provenance} />{metric.confidence ? <ConfidenceBadge compact confidence={metric.confidence} /> : null}</footer></article>)}</div>
      </section>
      <section className="executive-operating-grid">
        <article><header><span>AGENT WORKFORCE</span><Bot aria-hidden="true" /></header><strong>{workforce.total}</strong><p>{workforce.registeredSource === "FIXTURE" ? "fixture registry count · live telemetry UNAVAILABLE" : "registered agents"}</p><dl><div><dt>Working</dt><dd>{workforce.working}</dd></div><div><dt>Blocked</dt><dd>{workforce.blocked}</dd></div><div><dt>Waiting approval</dt><dd>{workforce.waitingApproval}</dd></div><div><dt>Completed / failed</dt><dd>{workforce.completed} / {workforce.failed}</dd></div></dl><footer><ExecutionRealityBadge reality={workforce.executionReality} /><span>{workforce.classification}</span><Link href="/agents">Workforce →</Link></footer></article>
        <article><header><span>EXPERIMENTS</span><FlaskConical aria-hidden="true" /></header><strong>{experiments.active} / {experiments.completed}</strong><p>active / completed · {experiments.classification}</p><b>{experiments.resultState.replaceAll("_", " ")}</b><small>{experiments.learning}</small><footer><ConfidenceBadge compact confidence={experiments.confidence} /><Link href={experiments.href}>Experiment Lab →</Link></footer></article>
        <article><header><span>LEARNING</span><Library aria-hidden="true" /></header><strong>{learning.memoryState.replaceAll("_", " ")}</strong><p>{learning.insight}</p><small>{learning.evidenceCount} evidence sources · {learning.classification} · privilege {learning.privilege.replaceAll("_", " ")}</small><footer><ConfidenceBadge compact confidence={learning.confidence} /><Link href={learning.href}>Memory →</Link></footer></article>
        <article><header><span>AI ECONOMICS</span><CircleDollarSign aria-hidden="true" /></header><strong>${economics.estimatedTaskCost.toFixed(2)}</strong><p>{economics.costKind} · not an invoice</p><dl><div><dt>Modeled budget</dt><dd>${economics.estimatedBudget}</dd></div><div><dt>Actual cost</dt><dd>{economics.actualCost}</dd></div><div><dt>Revenue</dt><dd>{economics.actualRevenue}</dd></div><div><dt>Verified ROI</dt><dd>{economics.verifiedRoi}</dd></div></dl><footer><ExecutionRealityBadge reality={economics.executionReality} /><Link href="/ai-cost">AI Economics →</Link></footer></article>
      </section>
    </div>
  );
}
