...
export function ExecutiveActivityCenter({ events }: { events: ExecutiveActivityEvent[] }) {
  ...
  return (
    <section className="executive-activity" aria-labelledby="executive-activity-title">
      ...
      <ol>{visible.length ? visible.map((event) => { const Icon = icons[event.severity]; return <li className={`executive-activity__event executive-activity__event--${event.severity.toLowerCase()}`} key={event.id}><time dateTime={event.timestamp}>{new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.timestamp))}</time><span><Icon aria-hidden="true" /></span><div><header><b>{event.domain}</b><strong>{event.title}</strong></header><p>{event.description}</p><small>{event.sourceId} · {event.status.replaceAll("_", " ")} · {event.classification}</small></div><aside><ExecutionRealityBadge reality={event.executionReality} /><DataSourceIndicator provenance={event.provenance} /></aside></li>; }) : <li className="executive-activity__empty" role="status">No events in this severity. Events are not invented to fill the stream.</li>}</ol>
    </section>
  );
}
